...
- **`src/utils.rs`** — interactive prompts and table rendering:
  - `ask_question` / `ask_question_mandatory` re-prompt in place with ANSI escapes (`\x1b[F\x1b[2K`) on invalid input; `ask_question_prefilled` (rustyline) additionally prefills the line with the current value for in-place editing (used by `edit`).
